...
function get_log(TorrentID,LogScore)
{
   if ($('#logs_' + TorrentID).raw().innerHTML == '')
   {
        $('#logs_' + TorrentID).raw().innerHTML = 'Loading, please wait!';
        show_log(TorrentID);
        //var start = new Date().getTime(); /* for benchmarking */
        ajax.get('torrents.php?action=log_ajax&torrentid=' + TorrentID + '&logscore=' + LogScore, function(response) {
           $('#logs_' + TorrentID).raw().innerHTML = response;
...
function show_log(TorrentID){
        $('#logs_' + TorrentID).gtoggle();
        $('#files_' + TorrentID).ghide();
        $('#peers_' + TorrentID).ghide();
        $('#snatches_' + TorrentID).ghide();
        $('#downloads_' + TorrentID).ghide();
