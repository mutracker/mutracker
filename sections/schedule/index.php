...
/// --- Update seedtimes  -- pprnts  (ps LOL HOW DO I INTO WEB DEVELOPMENT )//   pt1
	$DB->query(" 
	...
	*/
	if (count($UserIDs) > 0) {
		foreach ($UserIDs as $UserID) {  // find torrents that are seeded by < 5 seeders, give bonus points to those seeders
			//if( $UserID != 147 ) continue; // Check yoself b4 you shrek others
			$DB->query(" 
				SELECT u.seedtime, t.Seeders
			...
			$DB->query(" 
				SELECT bonusseedtime, Uploaded, Uploaded_last
				FROM users_main
				WHERE ID='$UserID'
			");
			list( $bonusseedtime, $Uploaded, $Uploaded_prev ) = $DB->next_record();
			$UploadedDiff = $Uploaded - $Uploaded_prev;
			$UploadedGB = (float)$UploadedDiff / 1024 / 1024 / 1024;
			$torrentCount = count($Seeders);
			$seedersCount = array_sum($Seeders);
			
//			$seedersum = array_sum( $Seeders );
//			echo "uploaded = $Uploaded <br/>";
//			echo "uploaded_prev = $Uploaded_prev <br/>";
//			echo "UploadedDiff = $UploadedDiff <br/>";
			
			$seederAvg = array_sum($Seeders) / count($Seeders);
			// for($i = 0; $i < $torrentCount; $i++)
			// {
				// $Plus += $seedtimes[$i] / ($Seeders[$i] * $Seeders[$i]);
			// }
			$totalbonustime = array_sum($seedtimes);
			$timediff = $totalbonustime - $bonusseedtime;
			$newcredits =  0.5 * ($timediff / 86400) / $seederAvg;    // magic 86400
			$newcredits = $newcredits + $UploadedGB * 5; 
			$time = $timediff / 86400;
			/*echo "seederAvg = $seederAvg <br/>";
			echo "time = $timediff <br/>";
			echo "torrentcount = $torrentCount <br/>";
			echo "seeders = $seedersCount <br/>";
			echo "uploadedGB = $UploadedGB <br/>";
	...
	//------------- Remove dead peers ---------------------------------------//
	sleep(3);
 	// back up rows from xbt_files_users into users_seeding_history before we delete them
    $DB->query("INSERT INTO users_seeding_history (
			SELECT u.uid, u.fid, IF(u.remaining = 0, TRUE, FALSE), u.downloaded, u.uploaded, u.timespent, FROM_UNIXTIME(u.mtime)
			FROM xbt_files_users AS u
			WHERE mtime < unix_timestamp(NOW() - INTERVAL 6 HOUR)
		) ON DUPLICATE KEY UPDATE 
			users_seeding_history.complete = VALUES(complete),
			users_seeding_history.downloaded = users_seeding_history.downloaded + VALUES(downloaded),
			users_seeding_history.uploaded = users_seeding_history.uploaded + VALUES(uploaded),
			users_seeding_history.timespent = users_seeding_history.timespent + VALUES(timespent),
			users_seeding_history.lastAction = VALUES(lastAction)
	");
...
if (!$NoDaily && $Day != $NextDay || $_GET['runday']) {
	...
	$RatioRequirements = array(
		array(80 * 1024 * 1024 * 1024, 0.0, 0.50),
		array(60 * 1024 * 1024 * 1024, 0.0, 0.40),
		array(50 * 1024 * 1024 * 1024, 0.0, 0.30),
		array(40 * 1024 * 1024 * 1024, 0.0, 0.20),
		array(30 * 1024 * 1024 * 1024, 0.0, 0.10),
		array(20 * 1024 * 1024 * 1024, 0.0, 0.05),
		array(10 * 1024 * 1024 * 1024, 0.0, 0.0),
		array(5 * 1024 * 1024 * 1024, 0.0, 0.0)
	);
	...
	$DB->query("
        ...
        //------------- Hit and Run / HnRs Update ------------------------------------//
        
		// insert hnrs for snatches 14 days ago.
		$DB->query("INSERT INTO users_hnrs (
			SELECT s.uid, s.fid, FROM_UNIXTIME(s.tstamp) as snatched, 345600 - SUM( u.timespent ) AS remaining
			FROM xbt_snatched AS s
			LEFT JOIN users_seeding_cumu AS u ON u.userID = s.uid AND u.torrentID = s.fid
			WHERE u.timespent < 345600 AND  s.tstamp <= UNIX_TIMESTAMP( NOW( ) - INTERVAL 14 DAY ) AND s.tstamp >= 1410616729
			GROUP BY s.uid, s.fid)
			ON DUPLICATE KEY UPDATE
				Remaining = VALUES(Remaining)");
        
        // update seed time for existing hnrs
		$DB->query("INSERT INTO users_hnrs (
			SELECT h.UserID, h.TorrentID, h.Snatched, 345600 - SUM( u.timespent ) AS remaining
			FROM users_hnrs AS h
			LEFT JOIN users_seeding_cumu AS u ON u.userID = h.UserID AND u.torrentID = h.TorrentID
			GROUP BY h.UserID, h.TorrentID)
			ON DUPLICATE KEY UPDATE
				Remaining = VALUES(Remaining)");

        // remove hnrs who have sufficient seeding time
		$DB->query("DELETE FROM users_hnrs WHERE Remaining <= 0");
		// or ratio
		$DB->query("DELETE FROM users_hnrs WHERE (UserID, TorrentID) IN
			(SELECT usc.userID as UserID, usc.torrentID as TorrentID FROM users_seeding_cumu as usc
			WHERE usc.uploaded >= usc.downloaded)");

        // dissable leeching privileges from users with 5 > HnRs
         //This SELECT here determines the users that will have their Snatching privs disabled
        $UserQuery =  $DB->query("SELECT UserID FROM users_hnrs GROUP BY UserID HAVING (count(UserID) > 5)");
        $ShityUsers = $DB->collect('UserID');

        if (count($ShityUsers) > 0) {
          
        $DB->query("
            UPDATE users_info AS ui
                JOIN users_main AS um ON um.ID = ui.UserID
                SET um.hnr_disabled = '1',
                ui.AdminComment = CONCAT('$sqltime - Leeching disabled by 5+ HnRs.\n\n', ui.AdminComment) 
            WHERE um.Enabled = '1' AND um.hnr_disabled = 0 AND ui.UserID IN(".implode(',', $ShityUsers).')');
        }

        foreach ($ShityUsers as $UserID) {
            $Cache->begin_transaction("user_info_heavy_$UserID");
            $Cache->update_row(false, array('CanLeech' => 0));
            $Cache->commit_transaction(0);
            echo "User Leeching privileges disabled: $UserID\n";
        }
        
        // enable leeching privs for users previously disabled 
        // only if they now have less or equal than 5 hnrs
        $DB->query("UPDATE users_info AS ui
                    JOIN users_main AS um ON um.ID = ui.UserID
                    SET
                      um.hnr_disabled=0 ,
                      ui.AdminComment = CONCAT('$sqltime - Leeching enabled after being disabled by 5+ HnRs.\n\n', ui.AdminComment)
                    WHERE um.hnr_disabled=1".((count($ShityUsers) > 0) ? " AND um.ID NOT IN (".implode(",", $ShityUsers).")" : ""));
        
      	//------------- HnR totals ------------------------------------//
        sleep(3);
        $DB->query("SELECT COUNT(*) as Total, COUNT(DISTINCT UserID) as HnRunners FROM users_hnrs");
        list($HnRTotal,$HnRunners) = $DB->next_record();
        $DB->query("INSERT IGNORE INTO users_hnrs_stat (Date, Total, HnRunners) VALUES(NOW(), $HnRTotal, $HnRunners)");
        $DB->query("DELETE FROM users_hnrs_stat WHERE Date < DATE(NOW() - INTERVAL 6 MONTH)"); //keep six months
