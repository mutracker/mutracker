...
switch ($_GET['type']) {
	case 'snatched':
		...
		$ExtraWhere = '';
		$From = "";
		if (check_perms('users_view_seedleech') || check_perms('users_override_paranoia')) {
			$elevated_perms = true;
			$ExtraField1 = 'uh.Remaining';
			$From = "xbt_snatched AS xs JOIN torrents AS t ON t.ID=xs.fid LEFT JOIN users_hnrs AS uh ON uh.TorrentID=t.ID AND uh.UserID=xs.uid";
		} else {
	...
	case 'downloaded':
		...
		break;
    case 'hnr':
		$elevated_perms = (check_perms('users_view_seedleech') || check_perms('users_override_paranoia')) ? true : false;
		if (!$elevated_perms && $UserID!=$LoggedUser['ID']) { error(403); }
		$ExtraField1 = 'uh.Remaining';
		$Time = 'unix_timestamp(uh.Snatched)';
		$UserField = 'uh.UserID';
		$ExtraWhere = '';
		$From = "users_hnrs AS uh LEFT JOIN torrents AS t ON t.ID=uh.TorrentID";
		break;
	default:
		error(404);
}
...
if ((empty($_GET['search']) || trim($_GET['search']) === '') && $Order != 'Name') {
	$SQL = "
		SELECT
			...
			t.ID AS TorrentID,
			$Time AS Time,
			tg.CategoryID, $ExtraField1 AS Extra1
		FROM $From
...
<div class="thin">
  ...
  <? if ($Action == 'hnr') { ?>
	<p class="center">You currently have <?=display_hnr($TorrentCount).' hnr'.($TorrentCount != 1 ? 's' : '')?>. <a href="wiki.php?action=article&name=hnr"><strong>Seed</strong></a> listed torrents to remove hit and runs.</p>
<? } else { ?>
	<div>
...
<?
	...
	foreach ($TorrentsInfo as $TorrentID => $Info) {
      ...
      list($GroupID,, $Time, $CategoryID, $Extra1) = array_values($Info);
...
<?					Votes::vote_link($GroupID, isset($UserVotes[$GroupID]) ? $UserVotes[$GroupID]['Type'] : ''); ?>
                ...
                <? if (in_array($Action, array('hnr','snatched')) && $elevated_perms === true) {
					$Extra1 = empty($Extra1) ? null : time() + $Extra1;
				?>
					<div class="tags">
						Remaining:&nbsp;<strong><?=($Extra1 ? time_diff($Extra1,3) : '---');?></strong>
					</div>
				<? } elseif (in_array($Action, array('upl_ttl','sna_ttl'))) { ?>
					<div class="tags">
						Last active:&nbsp;<strong><?=($Extra1 ? time_diff($Extra1,3) : '---');?></strong>
					</div>				
				<? } ?>
            </div>
			</td>
