...
list($Media, $Format, $Encoding, $Year, $GroupID, $Name, $Image, $CategoryID, $Size, $FreeTorrent, $InfoHash, $Uploader) = array_shift($Info); // used for generating the filename
$Artists = $Info['Artists'];

if(isset($LoggedUser))
{
  if (!$_REQUEST['usetoken'] && $FreeTorrent == '0') {
      if ($LoggedUser['Credits'] <= 0) {
          error('You cannot download this torrent without credits.');
      }
  }
}
else
...
if( $FreeTorrent == '0' && $UserID != $Uploader)
{
	if (!Torrents::has_token($TorrentID)) {
		$DB->query("
			INSERT INTO users_freeleeches (UserID, TorrentID, Time)
	...
	$DB->query("
		...
		$Cache->update_row(false, array('Credits' => $Credits));
		$Cache->commit_transaction(0);
