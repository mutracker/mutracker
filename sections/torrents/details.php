...
<?	} ?>
<?php if (!check_perms('site_edit_wiki') && check_perms('torrents_art_edit')): ?>
            <a href="torrents.php?action=editart&amp;groupid=<?=$GroupID?>" class="brackets">Edit album art</a>
<?php endif; ?>
...
<?
		if (check_perms('torrents_art_edit')) { ?>
...
<?	}?>
						...
						<? $Encoding2 = ($Encoding=="Lossless")?'':'/'.$Encoding; ?>
						| <a style="cursor:pointer;" onclick="setVisibility('sharebox','<?=$Format?><?=$Encoding2?>');" class="tooltip" title="Share">SH</a>
					]</span>
...
<?	} ?>
<? if($LogInDB) { ?>
          <a href="#" onclick="get_log('<?=$TorrentID?>','<?=$LogScore?>');return false;">[View Log]</a>                    
<? } ?>                    
                        
					</div>
                  <? if($LogInDB) { ?>
                  <div id="logs_<?=$TorrentID?>" class="hidden"></div>
                  <? } ?>
...
$Sharetext = $Title .' [_FORMAT] &#13>' ;
$taglist = '';
foreach ($Tags as $TagKey=>$Tag) {
	$taglist .= $Tag['name'] . ', ';
}
$Sharetext .= substr($taglist, 0, -2) . '&#13https://'. SSL_SITE_URL . '/torrents.php?id=' . $GroupID;
?>
<textarea style="display:none;" id="sharetext" cols="60" rows="5" tabindex="1"><?=$Sharetext?></textarea>
<script language="JavaScript">
function setVisibility(id, format) {
// if(document.getElementById(id).style.display == 'none'){

var text = document.getElementById('sharetext').innerHTML;
text = text.replace("_FORMAT", format);
...
<div style="display:inline;">
	<textarea name="body" id="sharebox" style="display:none;" cols="60" rows="5" tabindex="1"></textarea>
</div>
