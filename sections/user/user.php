...
// Hit and Runs
// This cross-references torrents/groups to avoid ghost entries, then caches result for a day
$HitAndRuns = $Cache->get_value('hit_and_run_count_'.$UserID);
if(!strlen($HitAndRuns))
{
	...
	list($HitAndRuns) = $DB->next_record();
	$HitAndRuns = is_numeric($HitAndRuns) ? $HitAndRuns : '0';
	//24 hours
	$Cache->cache_value('hit_and_run_count_'.$UserID, $HitAndRuns, 86400);
}
// End hnr
...
if (check_perms('users_mod')) { // Person viewing is a staff member
	$DB->query("
		SELECT
			...
			m.Paranoia,
			m.Invites,
			IF (m.can_leech = 1 AND m.hnr_disabled=0, 1, 0) AS CanLeech,
			m.Visible,
			i.JoinDate,
			i.Info,
			i.Avatar,
...
$DisableLeech, $JoinDate, $Info, $Avatar, $FLTokens, $Credits, $Donor, $Warned,
...
<li><span title="Hit and Runs">HnRs: </span><?=display_hnr($HitAndRuns)?>
<?		if ($HitAndRuns > 0) { ?>
				[<a href="torrents.php?type=hnr&amp;userid=<?=$UserID?>" title="View">View</a>]
<? 		} ?>
				</li>
                  <?
	}
...
?>
				...
				<li<?=($Override === 2 ? ' class="paranoia_override"' : '')?>>Credits: <?=$Credits?></li>
<?
	}
...
<?		foreach ($RecentSnatches as $RS) { ?>
			<td>
				<a href="torrents.php?id=<?=$RS['ID']?>">
					<img class="tooltip" 
                         title="<?=display_str($RS['Artist'])?><?=display_str($RS['Name'])?>" 
                         src="<?=ImageTools::process($RS['WikiImage'], true)?>" 
                         alt="<?=display_str($RS['Artist'])?><?=display_str($RS['Name'])?>" 
                         width="107" />
				</a>
			</td>
<?		} ?>
...
?>
			...
			<tr>
				...
				<td>
					<input type="text" size="5" name="Credits" value="<?=$Credits?>" />
				</td>
			</tr>
<?
	}
