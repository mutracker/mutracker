...
$ForceUserName = $Forums[$ForumID]['ForceUserName'];
$ForcedName = display_str($Forums[$ForumID]['ForcedName']);
...
<?	} ?>
	...
	</div><div id="hoverdiv" style="position:fixed; display:none;">eh</div>
<?
} //End Polls
			...
			<div style="float: left;"><a class="post_id" href="forums.php?action=viewthread&amp;threadid=<?=$ThreadID?>&amp;postid=<?=$PostID?>#post<?=$PostID?>">#<?=$PostID?></a>
				<? if($ForceUserName)
				{
					if( check_perms('site_moderate_forums')) { 
						echo "<a href=\"user.php?id=$AuthorID\">$ForcedName</a>"; 
						echo "<span style=\"display:none;\">";
						echo Users::format_username($AuthorID, true, true, true, true, true, $IsDonorForum);
						echo "</span>";
					}
					else { 
						echo "$ForcedName"; 
				...
				else if( $ThreadID == 429 ) 
				{
					if( check_perms('site_moderate_forums')) { 
						echo "<a href=\"user.php?id=$AuthorID\">tide</a>";
						echo "<span style=\"display:none;\">";
						echo Users::format_username($AuthorID, true, true, true, true, true, $IsDonorForum);
						echo "</span>";
					}
				...
				- <a href="#quickpost" id="quote_<?=$PostID?>" onclick="Quote('<?=$PostID?>', '<?=$ForceUserName ? $ForcedName : $Username?>', true);" class="brackets">Quote</a>
...
<?	if (Users::has_avatars_enabled()) { ?>
		...
		<?=$ForceUserName ? Users::show_avatar($Avatar, $AuthorID, $Username, -1, 150, true) : Users::show_avatar($Avatar, $AuthorID, $Username, $HeavyInfo['DisableAvatars'], 150, true)?>
		</td>
<?	} ?>
		<td class="body" valign="top"<? if (!Users::has_avatars_enabled()) { echo ' colspan="2"'; } ?>>
			<div id="content<?=$PostID?>">
				<?=Text::full_format($Body, true, 3) ?>
<?	if ($EditedUserID) { ?>
				<br />
				<br />
...
<?		} ?>
				Last edited by
				<?=$ForceUserName ? $ForcedName : Users::format_username($EditedUserID, false, false, false, false, false, $IsDonorForum) ?> <?=time_diff($EditedTime, 2, true, true)?>
<?	} ?>
			</div>
		</td>
	</tr>
