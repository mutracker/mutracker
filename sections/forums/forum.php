...
<?		} ?>
			<span style="float: right;" class="last_poster">
				by <? if($ForceUserName) { echo "$ForcedName";} else { echo Users::format_username($LastAuthorID, false, false, false, false, false, $IsDonorForum); }?> <?=time_diff($LastTime,1)?>
			</span>
		</td>
		<td class="number_column"><?=number_format($PostCount - 1)?></td>
		<td><? if($ForceUserName) { echo "$ForcedName";} else { echo Users::format_username($AuthorID, false, false, false, false, false, $IsDonorForum); }?></td>
	</tr>
<?	}
} ?>
</table>
