...
foreach ($Forums as $Forum) {
	list($ForumID, $CategoryID, $ForumName, $ForumDescription, $MinRead, $MinWrite, $MinCreate, $NumTopics, $NumPosts, $LastPostID, $LastAuthorID, $LastTopicID, $LastTime, $SpecificRules, $LastTopic, $Locked, $Sticky, $ForceUserName, $ForcedName) = array_values($Forum);
...
<? } ?>
			<span style="float: right;" class="last_poster">by <? if($ForceUserName) { echo "$ForcedName";} else { echo Users::format_username($LastAuthorID, false, false, false) ;}?> <?=time_diff($LastTime, 1)?></span>
		</td>
