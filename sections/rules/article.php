...
if (!empty($_GET['id']) && is_number($_GET['id'])) { //Visiting article via ID
	...
	$ArticleID = Rules::alias_to_id($_GET['name']);
} else { //No ID, No Name
	error(404);
	error('Unknown article ['.display_str($_GET['id']).']');
	$ArticleID = 1;//Rules::alias_to_id('Rules');
}
...
<div class="thin">
	...
	<div class="box pad" style="padding: 10px 10px 10px 20px;">
		...
		<ul>
			<li><a href="rules.php?action=search&amp;search=<?=display_str($_GET['name'])?>">Search</a> for an article similar to this.</li>
			<li><a href="rules.php?action=create&amp;alias=<?=display_str(Rules::normalize_alias($_GET['name']))?>">Create</a> an article in its place.</li>
		</ul>
	</div>
</div>
<?
...
$Article = Rules::get_article($ArticleID);
...
<div class="thin">
	<div class="header">
		<h2><?=$Title?></h2>
		<div class="linkbox">
<? if (check_perms('site_edit_rules')) { ?>
			<a href="rules.php?action=create" class="brackets">Create</a>
			<a href="rules.php?action=edit&amp;id=<?=$ArticleID?>" class="brackets">Contribute</a>
			<a href="rules.php?action=revisions&amp;id=<?=$ArticleID?>" class="brackets">History</a>
<? } ?>
<? if (check_perms('admin_manage_rules') && $_GET['id'] != INDEX_ARTICLE) { ?>
			<a href="rules.php?action=delete&amp;id=<?=$ArticleID?>&amp;authkey=<?=$LoggedUser['AuthKey']?>" class="brackets" onclick="return confirm('Are you sure you want to delete?\nYes, DELETE, not as in \'Oh hey, if this is wrong we can get someone to magically undelete it for us later\' it will be GONE.\nGiven this new information, do you still want to DELETE this article and all its revisions and all its alias\' and act like it never existed?')">Delete</a>
<? } ?>
	...
	<div class="sidebar">
		<div class="box">
			...
			<div class="pad">
			<form class="search_form" name="articles" action="rules.php" method="get">
			...
			<a href="rules.php?action=browse" class="brackets">Browse articles</a>
		</div>
		</div>
		<div class="box">
...
?>
						<li id="alias_<?=$AliasItem?>"><a href="rules.php?action=article&amp;name=<?=$AliasItem?>"><?=Format::cut_string($AliasItem, 20, 1)?></a><? if (check_perms('admin_manage_rules')) { ?> <a href="#" onclick="Remove_Alias('<?=$AliasItem?>'); return false;" class="brackets tooltip" title="Delete alias">X</a> <a href="user.php?id=<?=$UserArray[$i]?>" class="brackets tooltip" title="View user">U</a><? } ?></li>
<?		$i++;
	}
}
?>
...
<? if ($Edit <= $LoggedUser['EffectiveClass']) { ?>
		<div class="box box_addalias">
			<div style="padding: 5px;">
				<form class="add_form" name="aliases" action="rules.php" method="post">
