...
<?
$Row = 'b';
while (list($ID, $CategoryID, $Sort, $Name, $Description, $MinClassRead, $MinClassWrite, $MinClassCreate, $AutoLock, $AutoLockWeeks, $ForcedName) = $DB->next_record()) {
	$Row = $Row === 'a' ? 'b' : 'a';
?>
	<tr class="row<?=$Row?>">
...
<?	} ?>
			...
			<td>
				<input type="text" name="forcedname" value="<?=$ForcedName?>" />
			</td>
			<td>
