...
if ($_POST['submit'] == 'Delete') { //Delete
	...
	$Val->SetFields('forcedname', '1', 'string', 'The forced name has a max length of 40 characters', array('maxlength' => 40, 'minlength' => -1));
	...
	if ($_POST['submit'] == 'Edit') { //Edit
		...
		$DB->query("
			...
			SET
				...
				ForcedName = '$P[forcedname]'
			WHERE ID = '$P[id]'");
	} else { //Create
		$DB->query("
			INSERT INTO forums
				(Sort, CategoryID, Name, Description, MinClassRead, MinClassWrite, MinClassCreate, AutoLock, AutoLockWeeks, ForceUserName, ForcedName)
			VALUES
				('$P[sort]', '$P[categoryid]', '$P[name]', '$P[description]', '$P[minclassread]', '$P[minclasswrite]', '$P[minclasscreate]', '$P[autolock]', '$P[autolockweeks]', '$ForceUserName', '$P[forcedname]')");
	}
}
