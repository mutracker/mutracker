...
if(!file_exists('GeoLiteCity-latest.zip'))
    shell_exec('wget http://geolite.maxmind.com/download/geoip/database/GeoLiteCity_CSV/GeoLiteCity-latest.zip');

exec('unzip -u GeoLiteCity-latest.zip');

//get uncompressed files names
$output = array();
exec('unzip -l GeoLiteCity-latest.zip', $output);
$files = array();
foreach($output as $out)
{
    if(preg_match('/^\s+[0-9]+\s+[0-9]{4}\-[0-9]{2}\-[0-9]{2}\s+[0-9]{2}\:[0-9]{2}\s+/', $out))
    {
        $out = trim(preg_replace('/\s+/', ' ', $out));
        $file = explode(' ', $out)[3];
        // we suppouse that only the date changes 
        // it will have to be fixed if the wirds Blocks and 
        // Locations are not found in the file name
        if(strpos($file, 'Blocks') !== false){
            $files['blocks'] = $file;
        }
        else if(strpos($file, 'Location') !== false){
            $files['locations'] = $file;
        }        
    }
}

if (($Locations = file($files['locations'], FILE_IGNORE_NEW_LINES)) === false) {
...
if (($Blocks = file($files['blocks'], FILE_IGNORE_NEW_LINES)) === false) {
	echo 'Error';
}
array_shift($Blocks);
array_shift($Blocks);
