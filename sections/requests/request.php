...
?>
				<tr>
					...
					<td class="number_column">
						<?=($Boldify ? '<strong>' : '') . ($User['Bounty'])  . ($Boldify ? "</strong>\n" : "\n")?>
					</td>
				</tr>
<?	}
...
<?	if ($CanVote) { ?>
					...
					<strong>Costs <?=$MinimumVote?> Credits</strong>
<?	} ?>
				</td>
			</tr>
...
?>
			<tr id="voting">
				<td class="label tooltip" title="These units are in base 2, not base 10. For example, there are 1,024 MB in 1 GB. But we credits now.">Custom vote</td>
				<td>
			...
			<tr>
				...
				<td>
					<form class="add_form" name="request" action="requests.php" method="get" id="request_form">
						...
						<input type="hidden" id="current_credits" value="<?=$LoggedUser['Credits']?>" />
						...
						Bounty after tax: <strong><span id="bounty_after_tax">0 Credits</span></strong><br />
						If you add the entered <strong><span id="new_bounty">0 Credits</span></strong> of bounty, your new credits will be: <b>
						<span id="new_credits"><?=(int)$LoggedUser['Credits']?></span><br /></b>
						<span id="new_ratio" class="hidden"><?=Format::get_ratio_html($LoggedUser['BytesUploaded'],$LoggedUser['BytesDownloaded'])?></span>
						<input type="button" id="button" value="Vote!"  onclick="Vote();" disabled />
					</form>
				</td>
			</tr>
<?	} ?>
			<tr id="bounty">
				...
				<td id="formatted_bounty"><?=(int)$RequestVotes['TotalBounty'] ?> Credits</td>
			</tr>
<?
	if ($IsFilled) {
