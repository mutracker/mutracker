...
if ( false /*$RequestVotes['TotalBounty'] > $Uploaded */) {
	...
	$DB->query('
		UPDATE users_main
		SET Credits = Credits - '.$RequestVotes['TotalBounty']."
		WHERE ID = $FillerID");
}
...
$DB->query("SELECT Credits FROM users_main WHERE ID='$FillerID'");
list($newcredits) = $DB->next_record();

$Cache->begin_transaction('user_info_'.$FillerID);
