	...
	<tr class="colhead_dark">
		<td colspan="<?=Users::has_avatars_enabled() ? 2 : 1?>">
			<div style="float: left;"><a class="post_id" href="forums.php?action=viewthread&amp;threadid=<?=$ThreadID?>&amp;postid=<?=$PostID?>#post<?=$PostID?>">#<?=$PostID?></a>
				<? if($ForceUserName)
				{
					if( check_perms('site_moderate_forums')) { echo "<a href=\"user.php?id=$AuthorID\">$ForcedName</a>"; }
					else echo "$ForcedName";
				}
				else if( $ThreadID == 429 ) 
				{
					if( check_perms('site_moderate_forums')) { echo "<a href=\"user.php?id=$AuthorID\">tide</a>"; }
					else echo "tide"; 
				} 
				else 
				{ 
				...
				- <a href="#quickpost" id="quote_<?=$PostID?>" onclick="Quote('<?=$PostID?>', '<?=$ForceUserName? 'anonymous': $Username?>', true);" class="brackets">Quote</a>
...
<?	} ?>
		<td class="body" valign="top"<? if (!Users::has_avatars_enabled()) { echo ' colspan="2"'; } ?>>
			<div id="content<?=$PostID?>">
				<?=Text::full_format($Body, true, 3, $ForceUserName) ?>
<?	if ($EditedUserID) { ?>
				<br />
				<br />
...
<?		} ?>
				Last edited by
				<?=$ForceUserName? 'anonymous':Users::format_username($EditedUserID, false, false, false, false, false, $IsDonorForum) ?> <?=time_diff($EditedTime, 2, true, true)?>
<?	} ?>
			</div>
		</td>
	</tr>
