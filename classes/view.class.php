...
class View {
	...
	public static function show_header($PageTitle = '', $JSIncludes = '', $CSSIncludes = '') {
		...
		if (!is_array(G::$LoggedUser) || empty(G::$LoggedUser['ID'])) {
			...
			if( (G::$LoggedUser['StyleName'] == 'mutracker') OR (G::$LoggedUser['StyleName'] == 'memetracker') ) {
				require(SERVER_ROOT.'/design/jebbis/privateheader.php');
			} else {
	...
	public static function show_footer($Options = array()) {
		...
		if (!is_array(G::$LoggedUser)) {
			...
			if( (G::$LoggedUser['StyleName'] == 'jebbis') OR (G::$LoggedUser['StyleName'] == 'memetracker') ) {
				require(SERVER_ROOT.'/design/jebbis/privatefooter.php');
			} else {
