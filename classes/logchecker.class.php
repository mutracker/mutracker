...
class LOG_CHECKER {
	...
	function new_file($Log) { $this->Log = $Log; }
	...
	function parse() {
		...
		if (preg_match("/^dBpoweramp /i", $Log) && $this->ProcessDBpoweramp) {
			...
			return array($this->Score,$this->Good,$this->Bad,$this->Log);
		}
		...
		foreach ($this->Logs as $key => $value) {
			if (trim($value) == "") { unset($this->Logs[$key]); } //strip empty
			//append stat msgs
		...
		if (count($this->Logs) > 1) { $this->Combined = count($this->Logs); } //is_combined
		...
		foreach ($this->Logs as $LogArrayKey => $Log) {
			...
			if(!$EAC && !$XLD){
				if ($this->Combined) { unset($this->Bad); $this->Bad[] = "Combined Log (".$this->Combined.")"; $this->Bad[] = "Unrecognized log file (".$this->CurrLog.")! Feel free to report for manual review."; }
				else { $this->Bad[] = "Unrecognized log file! Feel free to report for manual review."; }
				$this->Score = 1;
				return array($this->Score,$this->Good,$this->Bad,$this->Log);
			}
			else {
			...
			if (!$Count) { $this->account('Could not verify used drive',1); }

			$Log = preg_replace_callback(
				"/Media type( +): (.+)/i",
				array($this, 'media_type_xld'),
				$Log, 1, $Count);
			
			if ($XLD && $this->VERSION && $this->VERSION >= 20130127 && !$Count) { $this->account('Could not verify media type',1); }

			$Log = preg_replace_callback(
				'/Read mode( +): ([a-z]+)(.*)?/i',
				array($this,'read_mode'),
				$Log, 1, $Count);
			
			if (!$Count && $EAC) { $this->account('Could not verify read mode',1); }
			
			$Log = preg_replace_callback(
				'/Ripper mode( +): (.*)/i',
				array($this,'ripper_mode_xld'),
				$Log, 1, $XLDRipperMode);
			
			$Log = preg_replace_callback(
				'/Use cdparanoia mode( +): (.*)/i',
				array($this,'cdparanoia_mode_xld'),
			...
			$Log = preg_replace_callback(
				'/Max retry count( +): (\d+)/i',
				array($this,'max_retry_count'),
				$Log, 1, $Count);
			
			if (!$Count && $XLD) { $this->account('Could not verify max retry count'); }
			
			$Log = preg_replace_callback(
				'/Utilize accurate stream( +): (Yes|No)/i',
				array($this,'accurate_stream'),
				$Log, 1, $EAC_ac_stream);
			
			$Log = preg_replace_callback(
				'/, (|NO )accurate stream/i',
				array($this,'accurate_stream_eac_pre99'),
			...
			$Log = preg_replace_callback(
				'/Defeat audio cache( +): (Yes|No)/i',
				array($this,'defeat_audio_cache'),
				$Log, 1, $EAC_defeat_cache);
			
			$Log = preg_replace_callback(
				'/ (|NO )disable cache/i',
				array($this,'defeat_audio_cache_eac_pre99'),
			...
			$Log = preg_replace_callback(
				'/Disable audio cache( +): (.*)/i',
				array($this,'defeat_audio_cache_xld'),
				$Log, 1, $Count);
			
			if (!$Count && $XLD) { $this->account('Could not verify defeat audio cache',1); }
			
			$Log = preg_replace_callback(
				'/Make use of C2 pointers( +): (Yes|No)/i',
				array($this,'c2_pointers'),
				$Log, 1, $C2);
			
			$Log = preg_replace_callback(
				'/with (|NO )C2/i',
				array($this,'c2_pointers_eac_pre99'),
				$Log, 1, $C2_EACpre99);
			...
			$Log = preg_replace_callback(
				'/Read offset correction( +): ([+-]?[0-9]+)/i',
				array($this,'read_offset'),
				$Log, 1, $Count);
			
			if (!$Count) { $this->account('Could not verify read offset',1); }
			...
			$Log = preg_replace_callback(
				'/Fill up missing offset samples with silence( +): (Yes|No)/i',
				array($this,'fill_offset_samples'),
				$Log, 1, $Count);
			...
			$Log = preg_replace_callback(
				'/Delete leading and trailing silent blocks[ \w]*( +): (Yes|No)/i',
				array($this,'delete_silent_blocks'),
				$Log, 1, $Count);
			
			if (!$Count && $EAC) { $this->account('Could not verify silent blocks',1); }
			
			$Log = preg_replace_callback(
				'/Null samples used in CRC calculations( +): (Yes|No)/i',
				array($this,'null_samples'),
				$Log, 1, $Count);
			
			if (!$Count && $EAC) { $this->account('Could not verify null samples'); }
			...
			$Log = preg_replace_callback(
				'/Gap handling( +): ([^\n]+)/i',
				array($this,'gap_handling'),
				$Log, 1, $Count);
			
			if (!$Count && $EAC) { $this->account('Could not verify gap handling'); }
			
			$Log = preg_replace_callback(
				'/Gap status( +): (.*)/i',
				array($this,'gap_handling_xld'),
				$Log, 1, $Count);
			
			if (!$Count && $XLD) { $this->account('Could not verify gap status'); }
			...
			$Log = preg_replace_callback(
				'/Add ID3 tag( +): (Yes|No)/i',
				array($this,'add_id3_tag'),
				$Log, 1, $Count);
			
			if (!$Count && $EAC) { $this->account('Could not verify id3 tag setting'); }
			...
			if ($Count) { $this->account('Ripped with compression offset',false,0); }
			...
			if (!$Count && $XLD) { $this->account('Could not verify album gain'); }
			...
			$Log = preg_replace_callback(
				'/( +)([0-9]{1,3})( +)\|( +)(([0-9]{1,3}:)?[0-9]{2}[\.:][0-9]{2})( +)\|( +)(([0-9]{1,3}:)?[0-9]{2}[\.:][0-9]{2})( +)\|( +)([0-9]{1,10})( +)\|( +)([0-9]{1,10})( +)\n/i',
				array($this,'toc'),
				$Log);
			...
			$Log = preg_replace(
				'/End of status report/i',
				'<span class="good">End of status report</span>',
				$Log);
			
			$Log = preg_replace(
				'/Track(\s*)Ripping Status(\s*)\[Disc ID: ([0-9a-f]{8}-[0-9a-f]{8})\]/i',
				'<strong>Track</strong>$1<strong>Ripping Status</strong>$2<strong>Disc ID: </strong><span class="log1">$3</span>',
				$Log
				);
			
			$Log = preg_replace(
				'/(All Tracks Accurately Ripped\.?)/i',
				'<span class="good">$1</span>',
				$Log);
			
			$Log = preg_replace(
				"/\d+ track.* +accurately ripped\.? *\n/i",
			...
			//xld accurip summary
			$Log = preg_replace_callback(
				"/(Track +\d+ +: +)(OK +)\(A?R?\d?,? ?confidence +(\d+).*?\)(.*)\n/i",
				array($this,'ar_summary_conf_xld'),
				$Log);

			$Log = preg_replace_callback(
				"/(Track +\d+ +: +)(NG|Not Found).*?\n/i",
				array($this,'ar_summary_conf_xld'),
				$Log);
			...
			$Log = preg_replace(
				...
				"<span class=\"badish\">$1</span>\n", 
				$Log, -1, $Count);
			if ($Count) { $this->ARSummary['bad'] = $Count; }
			
			$Log = preg_replace(
				"/(Track +\d+ +.*?(could not|cannot) be verified as accurate.*)\n/i",
				"<span class=\"badish\">$1</span>\n",
				$Log, -1, $Count);
			if ($Count) { $this->ARSummary['bad'] = $Count; } //don't mind the actual count
			...
			while(list($Key, $TrackBody) = each($TrackBodies)) {
				...
				if (!$this->Range) { $TrackBody = '<span class="log5">Track</span>'.$Spaces.'<span class="log4 log1">'.$TrackNumber.'</span>'.$TrackBody; }
				else { $TrackBody = $Spaces.'<span class="log5">Filename</span> <span class="log4 log3">'.$TrackNumber.'</span>'.$TrackBody; }
				
				$TrackBody = preg_replace(
					'/Filename ((.+)?\.(wav|flac|ape))\n/is', /* match newline for xld multifile encodes */
					"<span class=\"log4\">Filename <span class=\"log3\">$1</span></span>\n",
					$TrackBody, -1, $Count);
				
				if (!$Count && !$this->Range) { $this->account_track('Could not verify filename',1); }
				...
				if (!$Count && $XLD) { $this->account_track('Could not verify read errors'); }
				
				$TrackBody = preg_replace_callback(
					'/(Skipped \(treated as error\))( +:) (\d+)/i',
					array($this,'xld_stat'),
					$TrackBody, -1, $Count);
				...
				$TrackBody = preg_replace_callback(
					'/(Edge jitter error \(maybe fixed\))( +:) (\d+)/i',
					array($this,'xld_stat'),
					$TrackBody, -1, $Count);
				...
				$TrackBody = preg_replace_callback(
					'/(Atom jitter error \(maybe fixed\))( +:) (\d+)/i',
					array($this,'xld_stat'),
					$TrackBody, -1, $Count);
				...
				$TrackBody = preg_replace_callback(
					'/(Dropped bytes error \(maybe fixed\))( +:) (\d+)/i',
					array($this,'xld_stat'),
					$TrackBody, -1, $Count);
				...
				$TrackBody = preg_replace_callback(
					'/(Inconsistency in error sectors)( +:) (\d+)/i',
					array($this,'xld_stat'),
					$TrackBody, -1, $Count);
				...
				if($Count) { $this->account_track('Suspicious position(s) found',20); }
				...
				if($Count){ $this->account_track('Timing problem(s) found',20); }
				...
				if($Count){ $this->account_track('Missing sample(s) found',20); }
				...
				if(!$EACTC && !$XLDTC && !$Aborted){
					...
					if(!$this->SecureMode){
						if ($EAC) { $Msg = 'Rip was not done in Secure mode, and T+C was not used - as a result, we cannot verify the authenticity of the rip (-40 points)'; }
						else if ($XLD) { $Msg = 'Rip was not done with Secure Ripper / in CDParanoia mode, and T+C was not used - as a result, we cannot verify the authenticity of the rip (-40 points)'; }
				...
				if ($matches) { $this->ARTracks[$TrackNumber] = $matches[1]; }
				else { $this->ARTracks[$TrackNumber] = 0; } //no match - no boost
				
				$TrackBody = str_replace(
					'Copy finished',
					'<span class="log3">Copy finished</span>',
					$TrackBody);
			...
			if ($this->Score != 100) { //boost?
				$boost = null; $minConf = null;
				if (!$this->ARSummary) {
					foreach($this->ARTracks as $Track => $Conf) {
						if (!is_numeric($Conf) || $Conf < 2) { $boost = 0; break; } //non-ar track found
						else { $boost = 1;  $minConf = (!$minConf  || $Conf < $minConf) ? $Conf : $minConf; }
					}
				} elseif (isset($this->ARSummary['good'])) { //range with minConf
					foreach($this->ARSummary['good'] as $Track => $Conf) {
						if (!is_numeric($Conf)) { $boost = 0; break; }
						else { $boost = 1;  $minConf = (!$minConf  || $Conf < $minConf) ? $Conf : $minConf; }
					}
					if (isset($this->ARSummary['bad']) || isset($this->ARSummary['goodish'])) { $boost = 0; } //non-ar track found
				}
				if ($boost) {
			...
			$this->ARTracks = array(); $this->ARSummary = array(); $this->DecreaseBoost = 0;
			$this->SecureMode = true; $this->NonSecureMode = null;
		} //end log loop
		...
		if ($this->Combined) { array_unshift($this->Bad, "Combined Log (".$this->Combined.")"); } //combined log msg
	...
	function drive($Matches){
		...
		$FakeDrives = array('Generic DVD-ROM SCSI CdRom Device');
	...
	function parse_dBpoweramp(){
		...
		foreach ($this->Logs as $key => $value) {
			if (trim($value) == "") { unset($this->Logs[$key]); continue; } //strip empty
			//prepend capture
		...
		$Log = preg_replace(
			"/dBpoweramp Release ([\.\d]+)(.*?)(\d+.*)/i",
			"dBpoweramp Release <span class=\"log1\">$1</span>$2<span class=\"log1\">$3</span>", 
			$Log, 1, $Count);

		//drive
		$Log = preg_replace_callback(
			"/Ripping with drive(.*?):( +\[.+\])/i",
			array($this, 'drive'),
			$Log, 1, $Count);
		...
		//overread
		$Log = preg_replace(
			'/Overread Lead-in\/out( *): (Yes|No)/i',
			'<span class="log5">Overread Lead-in/out$1</span>: <span class="log4">$2</span>',
			$Log, 1, $Count);

		//accurateRip active
		$Log = preg_replace_callback(
			'/AccurateRip:( +)([\w\s]+)/i',
			array($this,'dbpa_accuraterip_active'),
			$Log, 1, $Count);

		if (!$Count) { $this->account('Could not verify accurate rip active',1); }

		//c2 pointers
		$Log = preg_replace_callback(
			'/Using C2:( +)(Yes|No)/i',
			array($this,'dbpa_c2_pointers'),
			$Log, 1, $C2);

		if (!$C2) { $this->account('Could not verify c2 pointers',1); }

		//cache (potentially bad)
		$Log = preg_replace_callback(
			'/Cache:( +)(None|\w+)/i',
			array($this,'dbpa_audio_cache'),
			$Log, 1, $Cache);
		...
		$Log = preg_replace_callback(
			'/FUA Cache Invalidate:( +)(Yes|No)/i',
			array($this,'dbpa_fua'),
			$Log, 1, $Fua);

		if (!$Fua) { $this->account('Could not verify fua setting'); }

		//passes drive speed
		$Log = preg_replace(
			"/(Pass \d+ Drive Speed)(: +)([^,]+)/i",
			"<span class=\"log5\">$1</span>$2<span class=\"log4\">$3</span>",
			$Log, -1, $Count);

		//ultra
		$Log = preg_replace_callback(
			'/Ultra:+( +Vary Drive Speed: +)(Yes|No)(.*?Min Passes: +)(\d+)(.*?Max Passes: +)(\d+)(.*?Finish After Clean Passes: +)(\d+)/i',
			array($this,'dbpa_ultra'),
			$Log, 1, $Ultra);
		...
		//bad sector re-rip
		$Log = preg_replace_callback(
			'/Bad Sector Re-rip:+( +Drive Speed: +)(\w+)(.*?Maximum Re-reads: +)(\d+)/i',
			array($this,'dbpa_rerip'),
			$Log, 1, $ReRip);

		if (!$ReRip) { $this->account('Could not verify bad sector re-rip setting',1); }
		...
		if ($Log = preg_replace(
			"/(DSP Effects.*?)(:.*?)\n/i",
			"<span class=\"log5\">$1</span>$2\n",
			$Log, 1, $Matches)) {
			$badDSP = array( //add destructive effect names here, ci, partial
				'Normalize','Fade','Silence Track Deletion','Silence Removal',
				'Grabber','Equalizer','Insert Audio','Karaoke','Loop','Maximum Length',
				'Minimum Length','Reverse','Trim','Write Silence'
			);
			$dubiousDSP = array( //add potentially bad effect names here, ci, partial
				'Bit Depth','Channel Count','Channel Mapper','DirectX','Resample',
				'VST'
			);
		...
		if (!$Count) { $this->account('No extraction log',false,0,true); }
		...
		if ($Count) { $this->account('Uncheck over-read option for this drive',false,false,false,true); }
		...
		while(list($Key, $TrackBody) = each($TrackBodies)) {
			...
			$TrackBody = preg_replace_callback(
				'/( +ERROR Ripping LBA +)(\d+)( +to +)(\d+)(.*?\.)/i',
				array($this,'dbpa_lba'),
				$TrackBody, 1, $LBABad);

			if (!$LBAGood && !$LBABad) { $this->account_track('Could not verify rip status',30); }

			//filename
			$TrackBody = preg_replace(
				'/Filename: ((.+)?\.(wav|flac|ape|ignore))\n/i',
				"<span class=\"log4\">Filename: <span class=\"log3\">$1</span></span>\n",
				$TrackBody, 1, $Count);
				
			if (!$Count) { $this->account_track('Could not verify filename',1); }

			//ar track + conf (not in use atm for dbpa)
			preg_match('/AccurateRip: +Accurate +\(confidence ([0-9]+)\)/i', $TrackBody, $matches);
			if ($matches) { $this->ARTracks[$TrackNumber] = $matches[1]; }
			else { $this->ARTracks[$TrackNumber] = 0; }

			//we need this one instead
			preg_match('/AccurateRip: +Inaccurate +\(confidence ([0-9]+)\)/i', $TrackBody, $matches);
			if ($matches) { $this->IARTracks[$TrackNumber] = $matches[1]; } //match - no boost
			else { $this->IARTracks[$TrackNumber] = 0; }

			//accurate rip
			$TrackBody = preg_replace_callback(
				'/AccurateRip:(.*?)\(confidence ([0-9]+)\)/i',
				array($this,'dbpa_accurip'),
				$TrackBody, 1, $Count);

			//accurip crc
			$TrackBody = preg_replace(
				'/AccurateRip CRC: +([a-fA-F0-9]{8})/i',
				"<span class=\"log4\">AccurateRip CRC: <span class=\"log3\">$1</span></span>",
				$TrackBody, 1, $Count);

			//accurip verified
			$TrackBody = preg_replace_callback(
				'/AccurateRip(.+)(\[CRCv\d +)([a-fA-F0-9]+)\]/i',
				array($this,'dbpa_accurip_verified'),
				$TrackBody, -1, $Count);

			//copy crc
			$TrackBody = preg_replace(
				'/CRC32: +([a-fA-F0-9]{8})/i',
				"<span class=\"log4\">CRC32: <span class=\"log3\">$1</span></span>",
				$TrackBody, 1, $Count);
			...
			$TrackBody = preg_replace(
				'/\[DiscID: +(.*?)\]/i',
				"[DiscID: <span class=\"log1\">$1</span>]",
				$TrackBody, 1, $Count);

			//in/secure
			$TrackBody = preg_replace_callback(
				'/(I?n?Secure)(.*?)(\[.*?\])/i',
				array($this,'dbpa_secure'),
				$TrackBody, 1, $Count);

			//re-rip frame
			$TrackBody = preg_replace_callback(
				'/Re-rip Frame: +(.*?\n)/i',
				array($this,'dbpa_rerip_frame'),
				$TrackBody, -1, $Count);

			$DubiousTrack = false;
			if ($Count && !$this->InvalidateCache && $this->ARTracks[$TrackNumber] < 2) { $DubiousTrack = true; }
		...
		foreach($this->Tracks as $Num => $Track){ //send score/bad
			...
			if($Track['dubious']){ $this->DubiousTracks++; }
			//boost track
		...
		//boost	ar	
		$ar_with_conf = false; $minConf = null;
		foreach($this->ARTracks as $Track => $Conf) {
			if (!is_numeric($Conf) || $Conf < 2) { $ar_with_conf = false; break; } //non-ar track found
			else { $ar_with_conf = true;  $minConf = (!$minConf  || $Conf < $minConf) ? $Conf : $minConf; }
		}
		...
		$this->ARTracks = array(); $this->IARTracks = array(); $this->DecreaseBoost = 0;
		$this->SecureMode = true; $this->NonSecureMode = null; $this->InvalidateCache = true;
		} //end log loop
	...
	function dbpa_rerip_frame($Matches){
		...
		$Runs = null;
		if (preg_match('/\d+ +\/ +(\d+)/i', $Matches[1], $M)) { $Runs = $M[1]; }
	...
	function dbpa_status_report($Matches){
		...
		if ($Stopped || $Error || $Inaccurate || $Insecure) { $Class = "bad"; }
		else if ($Warning) { $Class = "badish"; }
		else if (($Secure && !$this->SecureMode) || ($Accurate && $Secure)) { $Class = "goodish"; }
		else if ($Accurate || ($Secure && $this->SecureMode)) { $Class = "good"; }
	...
	function check_tracks() {
		if (!count($this->Tracks)) { //no tracks
			unset($this->Bad);
			if ($this->Combined) { $this->Bad[] = "Combined Log (".$this->Combined.")"; $this->Bad[] = "Invalid log (".$this->CurrLog."), no tracks!"; }
			else { $this->Bad[] = "Invalid log, no tracks!"; }
			$this->Score = 0;
			return array($this->Score,$this->Good,$this->Bad,$this->Log);
		}
	}
	...
	function format_report() { //sort by importance & reasonable log length
		if (!count($this->Bad)) { return; }

		$myBad=array();
		foreach ($this->Bad as $Key => $Val) {
			if (preg_match("/(points?\W)|(boosted)\)/i", $Val)) { $myBad['high'][]=$Val; }
			else { $myBad['low'][]=$Val; }
		}
		$this->Bad = array(); $this->Bad = $myBad['high'];
		if (count($this->Bad) < $this->Limit) {
			foreach ($myBad['low'] as $Key => $Val) {
				if (count($this->Bad) > $this->Limit) { break; }
				else { $this->Bad[] = $Val; }
			}
		}
	...
	function account($Msg,$Decrease=false,$Score=false,$InclCombined=false,$Notice=false,$DecreaseBoost=false) {
		...
		if (!in_array($Prepend.$Msg.$Append1.$Append2, $this->Bad)) {
			...
			if ($DecreaseScore) { $this->Score-=$Decrease; }
			if ($SetScore) { $this->Score=$Score; }
			if ($DecreaseBoost) { $this->DecreaseBoost+=$DecreaseBoost; }
		}
	}
