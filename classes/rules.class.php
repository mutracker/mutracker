...
class Rules {
	...
	public static function get_aliases() {
		$Aliases = G::$Cache->get_value('rules_aliases');
		if (!$Aliases) {
			...
			G::$Cache->cache_value('rules_aliases', $Aliases, 3600 * 24 * 14); // 2 weeks
		}
		return $Aliases;
	}
	...
	public static function flush_aliases() {
		G::$Cache->delete_value('rules_aliases');
	}
	...
	public static function get_article($ArticleID, $Error = true) {
		$Contents = G::$Cache->get_value('rules_article_'.$ArticleID);
		if (!$Contents) {
			$QueryID = G::$DB->get_query_id();
			G::$DB->query("
				SELECT
				...
				FROM rules_articles AS w
					LEFT JOIN rules_aliases AS a ON w.ID=a.ArticleID
			...
			G::$DB->set_query_id($QueryID);
			G::$Cache->cache_value('rules_article_'.$ArticleID, $Contents, 3600 * 24 * 14); // 2 weeks
		}
		return $Contents;
	}
	...
	public static function flush_article($ArticleID) {
		G::$Cache->delete_value('rules_article_'.$ArticleID);
	}
