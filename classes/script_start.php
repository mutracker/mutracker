...
if ($SessionID && MODS_ONLY) {
	if( !check_perms('users_mod') )/* && (G::$LoggedUser['ID'] != 2499)  )*/ { // Right, I've allowed the first 20 people in for feedback n shit
		require(SERVER_ROOT.'/sections/index/soon.php');
	} else {
...
function display_hnr($Count, $Color = true)
{
	...
	if($Color)
	{
		...
		$Count = '<span class="'.$Rank.'">'.$Count.'</span>';
	}
	return $Count;
}
