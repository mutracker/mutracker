...
class Text {
	 ...
	 * @var array $ValidTags
	 */
	private static $ValidTags = array('b'=>0, 'u'=>0, 'i'=>0, 's'=>0, '*'=>0, '#'=>0, 'artist'=>0,'tag'=>0, 'label'=>0, 'youtube'=>0, 'youtube'=>0,'user'=>0, 'n'=>0, 'inlineurl'=>0, 'inlinesize'=>1, 'headline'=>1, 'align'=>1, 'color'=>1, 'colour'=>1, 'size'=>1, 'url'=>1, 'img'=>1, 'quote'=>1, 'pre'=>1, 'code'=>1, 'tex'=>0, 'hide'=>1, 'plain'=>0, 'important'=>0, 'torrent'=>0, 'rule'=>0, 'mature'=>1, 'spoiler' => 1,
	);
	...
	private static $Wordfilters = array(
		't_est'			=> 'test'
	);
	...
	public static function full_format($Str, $OutputTOC = true, $Min = 3) {
		...
		if (self::$TOC) {
			...
			$Str = preg_replace('/\bnigger\b/i', 'hipster', $Str);
			$Str = preg_replace('/\breddit\b/i', '4chan', $Str);
			$Str = preg_replace('/\b4chan\b/i', 'reddit', $Str);
			$Str = preg_replace('/\banime\b/i', 'chinese cartoons', $Str);
			$Str = preg_replace('/\bsucks\b/i', 'a shit', $Str);
			//$Str = preg_replace('/\ble\b/i', 'I SUCK COCK', $Str);
			$Str = preg_replace('/\bRommel\b/i', 'Hitler', $Str);
			$Str = preg_replace('/\bmeme\b/i', 'joke', $Str);
			$Str = preg_replace('/\bmemes\b/i', 'jokes', $Str);
			$Str = preg_replace('/\bwordfilter\b/i', 'freedom of speech', $Str);
			$Str = preg_replace('/\bshitposting\b/i', 'tom-foolery', $Str);
			
			//greentext
			$Str = preg_replace('/(\n|^)&gt;(.*?)(\n|$)/m', "$1[color=#63BD40]>$2[/color]$3", $Str);
		} else {
		...
		while ($i < $Len) {
			...
			switch ($TagName) {
				...
				case 'artist':
					...
					break;
				case 'tag':
					$Array[$ArrayPos] = array('Type'=>'tag', 'Val'=>$Block);
					break;
				case 'label':
					$Array[$ArrayPos] = array('Type'=>'label', 'Val'=>$Block);
					break;
				case 'youtube':
					$Array[$ArrayPos] = array('Type'=>'youtube', 'Val'=>$Block);
					break;						
				case 'torrent':
		...
		foreach ($Array as $Block) {
			...
			switch ($Block['Type']) {
				...
				case 'artist':
					...
					break;
				case 'tag':
					$Str .= '<a href="torrents.php?action=basic&taglist='.urlencode(Format::undisplay_str($Block['Val'])).'">'.$Block['Val'].'</a>';
					break;
				case 'label':
					$Str .= '<a href="torrents.php?action=basic&recordlabel='.urlencode(Format::undisplay_str($Block['Val'])).'">'.$Block['Val'].'</a>';
					break;
				case 'youtube':
				
					$VideoID = explode("?v=", $Block['Val']); // For videos like http://www.youtube.com/watch?v=...
					if (empty($VideoID[1]))
						$VideoID = explode("/v/", $Block['Val']); // For videos like http://www.youtube.com/watch/v/..
					if (empty($VideoID[1]))
						$VideoID = explode("u.be/", $Block['Val']);
					$VideoID = explode("&", $VideoID[1]); // Deleting any other params
					$VideoID = $VideoID[0];
					$Str .= '<iframe width="560" height="315" src="//www.youtube-nocookie.com/embed/'.$VideoID.'" frameborder="0" allowfullscreen></iframe>';
					break;
				...
				case 'quote':
					...
					if (!empty($Block['Attr'])) {
						...
						if (isset($Exploded[1]) && (is_numeric($Exploded[1]) || (in_array($Exploded[1][0], array('a', 't', 'c', 'r')) && is_numeric(substr($Exploded[1], 1))))) {
							...
							$Str .= '<a onmouseenter="hoverdiv(event,\''.$PostID.'\', true)" onmouseleave="hoverdiv(event,\''.$PostID.'\', false)" href="#" onclick="QuoteJump(event, \''.$PostID.'\'); return false;" >>>'.$PostID.'</a>';
							self::$NoImg--;
							self::$InQuotes--;
							break;
						}
