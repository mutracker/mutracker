...
// Main settings
define('SITE_NAME', 'mutracker'); //The name of your site
define('NONSSL_SITE_URL', 'mutracker.org'); //The FQDN of your site
define('SSL_SITE_URL', 'mutracker.org'); //The FQDN of your site, make this different if you are using a subdomain for ssl
define('SITE_IP', '81.92.221.103'); //The IP address by which your site can be publicly accessed
define('SERVER_ROOT', '/var/www'); //The root of the server, used for includes, purpose is to shorten the path string
define('ANNOUNCE_URL', 'http://'.NONSSL_SITE_URL.':36000'); //Announce URL
define('LASTFM_API_KEY', 'fb88a7f603e9a1f703ba286ab74ac9c5' );
...
// Keys
define('ENCKEY', 'LcdCPQTfkItJZmtaRzsS'); //Random key. The key for encryption
define('SITE_SALT', 'fw9izbj27x484UjsJynU'); //Random key. Default site wide salt for passwords, DO NOT LEAVE THIS BLANK/CHANGE AFTER LAUNCH!
define('SCHEDULE_KEY', 'muschedule'); // Random key. This key must be the argument to schedule.php for the schedule to work.   // Not so random, plebeian..
define('RSS_HASH', 'BRJrAwQwR8Nc2dBahIVi'); //Random key. Used for generating unique RSS auth key.

// MySQL details
define('SQLHOST', 'localhost'); //The MySQL host ip/fqdn
define('SQLLOGIN', 'mutsql');//The MySQL login
define('SQLPASS', 'mutracksql'); //The MySQL password
...
$MemcachedServers = array(
	...
	array('host' => 'unix:///tmp/memcached.sock', 'port' => 0, 'buckets' => 1),
);
...
define('MODS_ONLY', false); //Only allow moderators/staff to login ~pprnts is gay lol

// Ocelot details
define('TRACKER_HOST', 'localhost');
define('TRACKER_PORT', 36000);
define('TRACKER_SECRET', 'aGTYHNBvfcdER$#EdsxcHUJY78IKMhgt'); // Must be 32 characters and match site_password in Ocelot's config.cpp
define('TRACKER_REPORTKEY', 'BYYYYY&6tfc6rcr6c*NOIJNI(9yb8ytv'); // Must be 32 characters and match report_password in Ocelot's config.cpp
...
define('USER_LIMIT', 12500); //The maximum number of users the site can have, 0 for no limit
define('STARTING_INVITES', 0); //# of invites to give to newly registered users
define('BLOCK_TOR', true); //Set to true to block Tor users
define('BLOCK_OPERA_MINI', true); //Set to true to block Opera Mini proxy
...
// IRC settings
define('BOT_NICK', 'mubot');
define('BOT_SERVER', 'irc.mutracker.org'); // IRC server address. Used for onsite chat tool.
define('BOT_PORT', 6667);
// define('BOT_CHAN', '#mutracker'/*.NONSSL_SITE_URL*/);
define('BOT_CHAN', '#mutracker');
define('BOT_ANNOUNCE_CHAN', '#announce');
define('BOT_STAFF_CHAN', '#muhtracker');
...
$Categories = array('Music', 'Music Literature', 'Audiobooks', 'Music Videos', 'Sheet Music');
...
$ReleaseTypes = array(1=>'Album', 3=>'Soundtrack', 5=>'EP', 6=>'Anthology', 7=>'Compilation',8 => 'DJ Mix', 9=>'Single', 11=>'Live album', 13=>'Remix', 14=>'Bootleg', 15=>'Interview', 16=>'Mixtape', 23 => 'Demo', 21=>'Unknown');
...
$RatioRequirements = array(
	array(50 * 1024 * 1024 * 1024, 0.00, date('Y-m-d H:i:s')),
	array(40 * 1024 * 1024 * 1024, 0.00, date('Y-m-d H:i:s')),
	array(30 * 1024 * 1024 * 1024, 0.00, date('Y-m-d H:i:s')),
	array(20 * 1024 * 1024 * 1024, 0.00, date('Y-m-d H:i:s')),
	array(10 * 1024 * 1024 * 1024, 0.00, date('Y-m-d H:i:s')),
	array(5 * 1024 * 1024 * 1024,  0.00, date('Y-m-d H:i:s', time() - (60 * 60 * 24 * 14)))
);
