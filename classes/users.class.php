...
class Users {
	...
	public static function user_heavy_info($UserID) {
		...
		if (empty($HeavyInfo)) {
            ...
            $HitAndRuns = G::$Cache->get_value('hit_and_run_count_'.$UserID);
            if(!strlen($HitAndRuns))
            {
                G::$DB->query("SELECT SQL_CALC_FOUND_ROWS
                    uh.TorrentID
                    ...
                    GROUP BY t.ID
                ");
                G::$DB->query("SELECT FOUND_ROWS()");
                list($HitAndRuns) = G::$DB->next_record();
                $HitAndRuns = is_numeric($HitAndRuns) ? $HitAndRuns : '0';
                //24 hours
                G::$Cache->cache_value('hit_and_run_count_'.$UserID, $HitAndRuns, 86400);
            }
            
            $HeavyInfo['HitAndRuns'] = $HitAndRuns;
